...
class WC_Catalog_Restrictions_Category_Admin {
    ...
    public static function instance() {
        if (!self::$instance) {
            $instance = new WC_Catalog_Restrictions_Category_Admin();
        }
    }
    ...
    function add_category_fields() {
        ...
        <div id="wc_catalog_restrictions_location" class="form-field">
            <?php if ($wc_catalog_restrictions->get_setting('_wc_restrictions_locations_enabled', 'yes') == 'yes') : ?>
                <?php
                ...
                <div id="wc_catalog_restrictions_locations_container" class="woocommerce_options_panel"  style="<?php echo ($location_restriction == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
                    <p class="form-field"><label for="wc_restrictions_locations"><?php _e('Target Locations', 'wc_catalog_restrictions'); ?></label>
                        <select name="wc_restrictions_locations[]" class="wc_restrictions_chosen_select" multiple="multiple" data-placeholder="<?php _e('Search for a country&hellip;', 'woocommerce'); ?>">
                            <?php woocommerce_catalog_restrictions_country_multiselect_options($current_locations); ?>
                        </select> <img class="help_tip" data-tip='<?php _e('Choose locations for this category.  Only users who select a matching location will be able to view and purchase products in this category.', 'wc_catalog_restrictions') ?>' src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" /></p>
                </div>
            <?php endif; ?>
        </div>
        <?php
        ...
        <tr class="form-field">
            ...
            <td>
                <div id="wc_catalog_restrictions" class="form-field">
                    ...
                    <img class="help_tip" data-tip='<?php _e('Use to Show To Everyone to force products in this category to always be displayed, regardless of the customers role. Use Show to Specific Roles to choose which roles will see products in this category.'); ?>' src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" />
                    <div id="wc_catalog_restrictions_roles_container" style="<?php echo ($restricted == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
                        <p class="title"><?php _e("Choose the roles that can view this product", 'wc_catalog_restrictions'); ?></p>
                        <?php $chunks = array_chunk($all_roles, ceil(count($all_roles) / 3), true); ?>
                        <?php foreach ($chunks as $chunk) : ?>
            ...
            <tr class="form-field">
                ...
                <td>
                    <div id="wc_catalog_restrictions_location" class="form-field">
                        ...
                        <img class="help_tip" data-tip='<?php _e('Use to Any Location to force products in this category to always be displayed, regardless of the customers location. Use Specific Locations to choose which locations prodcuts in this category will be enabled for.'); ?>' src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" />
