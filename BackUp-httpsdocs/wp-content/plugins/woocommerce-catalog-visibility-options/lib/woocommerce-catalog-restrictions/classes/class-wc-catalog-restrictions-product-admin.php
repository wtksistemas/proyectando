...
class WC_Catalog_Restrictions_Product_Admin {
    ...
    public static function instance() {
        if (!self::$instance) {
            $instance = new WC_Catalog_Restrictions_Product_Admin();
        }
    }
    ...
    function post_meta_boxes_setup() {
        ...
        wp_enqueue_script('chosen', $woocommerce->plugin_url() . '/assets/js/chosen/chosen.jquery' . $suffix . '.js', array('jquery'), $woocommerce->version, true);
        wp_enqueue_script('wc-product-restrictions-admin', $wc_catalog_restrictions->plugin_url() . 'assets/js/admin.js', array('chosen', 'jquery'));
    ...
    function meta_box($object, $box) {
        ...
        <div class="woocommerce_options_panel">
            ...
            <?php if ($wc_catalog_restrictions->get_setting('_wc_restrictions_locations_enabled', 'yes') == 'yes') : ?>
                <div class="options_group">
                    ...
                    <div id="wc_catalog_restrictions_locations_container" class="wc_restrictions_options_panel" style="<?php echo (get_post_meta($object->ID, '_wc_restrictions_location', true) == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
                        ...
                        <select name="wc_restrictions_locations[]" class="wc_restrictions_chosen_select" multiple="multiple" data-placeholder="<?php _e('Search for a country&hellip;', 'wc_catalog_restrictions'); ?>">
                            <?php woocommerce_catalog_restrictions_country_multiselect_options($current_locations); ?>
                            ?>
                        </select> <img class="help_tip" data-tip='<?php _e('Choose locations for this product.  Only users who select a matching location will be able to view and purchase this product.', 'wc_catalog_restrictions') ?>' src="<?php echo $woocommerce->plugin_url(); ?>/assets/images/help.png" />
                    </div>
                </div>
            <?php endif; ?>
            ...
            <div id="wc_catalog_restrictions_roles_container" class="wc_restrictions_options_panel" style="<?php echo (get_post_meta($object->ID, '_wc_restrictions', true) == 'restricted' ? 'display:block;' : 'display:none;'); ?>">
                <label><?php _e("Choose the roles that can view this product", 'wc_catalog_restrictions'); ?></label><br />
